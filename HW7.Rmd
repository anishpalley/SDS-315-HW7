...
voted_1996_summary = turnout %>%
  group_by(voted1996) %>%
  summarize(mean_voted_1996 = mean(GOTV_call))
...
ggplot(turnout) +
  geom_boxplot(aes(x=factor(voted1996), y = voted1998))
...
majorpty_summary = turnout %>%
  group_by(MAJORPTY) %>%
  summarize(mean_majorpty = mean(GOTV_call))
...
# a boxplot that shows both between and within-group variation
ggplot(turnout) +
  geom_boxplot(aes(x=factor(MAJORPTY), y = voted1998))
...
voted_1996_summary_matched = gotv_matched %>%
  group_by(voted1996) %>%
  summarize(mean_voted_1996_matched = mean(GOTV_call))
...
majorpty_summary_matched = gotv_matched %>%
  group_by(MAJORPTY) %>%
  summarize(mean_majorpty_matched = mean(GOTV_call))
